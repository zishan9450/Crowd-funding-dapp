...
function deleteCampaign(uint256 campaignId) public {
    require(msg.sender == campaigns[campaignId].owner, "Only the owner can delete this campaign.");
    require(campaigns[campaignId].isActive, "Campaign already inactive.");

    campaigns[campaignId].isActive = false;  // Set to inactive rather than delete
    // Other deletion logic if necessary
}
